...
public:
  ...
  int _lastWordStart;
  int _lastWordEnd;
  const CStateItem<xpu> *_prevStackState;
  const CStateItem<xpu> *_prevSepState;
  const CStateItem<xpu> *_prevState;
  int _nextPosition;
  ...
  CAction _lastAction;
  Feature _curFeat;
  DenseFeatureForward<xpu> _nnfeat;
  HiddenForward<xpu> _hidden;
  dtype _score;
  int _wordnum;
...
public:
  CAction action;
  const CStateItem<xpu> *item;
  dtype score;
  Feature feat;
  DenseFeatureForward<xpu> nnfeat;
  HiddenForward<xpu> hidden;
...
public:
  int operator()(const CScoredStateAction<xpu> &o1, const CScoredStateAction<xpu> &o2) const {
